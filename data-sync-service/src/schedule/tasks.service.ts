...
export class TasksService {
  ...
  async initCache() {
    ...
    if (!txn_batch_block_number) {
      txn_batch_block_number = await this.l1IngestionService.getTxnBatchBlockNumber() || this.configService.get('L1_START_BLOCK_NUMBER');
    }
     if (!state_batch_block_number) {
      state_batch_block_number = await this.l1IngestionService.getStateBatchBlockNumber() || this.configService.get('L1_START_BLOCK_NUMBER');
    }
    ...
    await this.cacheManager.set(TXN_BATCH, Number(txn_batch_block_number), { ttl: 0 });
    await this.cacheManager.set(STATE_BATCH, Number(state_batch_block_number), { ttl: 0 });
