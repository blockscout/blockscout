...
### Step 1 — Determine CHAIN_TYPE and scope
...
If the prompt also contains `--unstaged`, pass it as a script flag. This narrows the scope from "all changed files vs base" to "unstaged and untracked files only" (skips staged and committed-vs-base changes). If the flag is absent, run with the default scope.
...
```
.agents/agents/scripts/code-quality-check.sh -e CHAIN_TYPE=<value> --unstaged
```
...
The script automatically detects the environment (host or devcontainer), finds the target Elixir files via git, and runs three checks sequentially: `mix format --check-formatted`, `mix credo`, and `cspell`.
...
Use `unstaged files` for `<scope>` when the invocation included `--unstaged`; otherwise use `changed files`.
...
Rules:
- If the script outputs `NO_FILES`, return: "No unstaged or untracked Elixir files to check." when `--unstaged` was passed, otherwise "No changed Elixir files to check."
