...
4. **Discover this repo's conventions, then cross-check the overview against them.** Do not assume which convention files exist — look first:
   ...
   If `CONTRIBUTING.md` is present, don't read it in full — most of it governs the external contribution process (fork/branch flow, PR title/label formatting, what gets accepted), not the content of a change. Read `.claude/rules/contributing-essentials.md` instead: a curated subset covering naming conventions and runtime-vs-compile-time configuration — the parts that actually bear on whether an overview's approach is right.
