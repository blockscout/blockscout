...
# Inspect code-quality session

Analyze a code-quality subagent JSONL session file to produce a step-by-step
narrative of what the agent did, the outcome of each action, and whether the
agent followed the code-quality spec.
...
```bash
python3 .agents/skills/inspect-code-quality-session/scripts/parse-session.py "<session-file-path>"
```
...
> "Do NOT attempt to recover by running mix format, mix credo, cspell, or any
> other command directly."
...
- File count discrepancies (prompt vs. git-detected)
...
- Mention timestamps when they reveal something interesting (e.g., a 3-minute
  gap suggesting a long-running compilation inside the container).
