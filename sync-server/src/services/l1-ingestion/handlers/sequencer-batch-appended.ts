...
> = {
  ...
  storeEvent: async (entry, db, client?: Client) => {
    ...
    if (client) {
      const sql = 'INSERT INTO txn_batches(batch_index, hash, size, pre_total_elements, timestamp, inserted_at, updated_at) VALUES ($1, $2, $3, $4, $5, $6, $7)'
      const params = [
        entry.transactionBatchEntry.index,
        entry.transactionBatchEntry.l1TransactionHash,
        entry.transactionBatchEntry.size,
        entry.transactionBatchEntry.prevTotalElements,
        new Date(entry.transactionBatchEntry.timestamp * 1000),
        new Date(),
        new Date()
      ]
      try {
        const res = await client.query(sql, params);
        if (res) console.log('insert tx batch successfully');

      } catch (e) {
        console.error('insert tx batch failed', e);
