...
if Application.get_env(:explorer, :chain_type) == :arbitrum do
  defmodule Indexer.Fetcher.Arbitrum.Workers.Confirmations.Discovery.NewAndKnownConfirmationIncompleteDatabaseTest do
    ...
    # At the same time, the database holds a part of the data which the new
    # confirmation needs. The indexer writes a batch, the links of its rollup blocks,
    # and the blocks themselves in several steps. The discovery can read the parent
    # chain between those steps.
    #
    # The discovery handles the two events separately. Thus a missing part of a batch
    # postpones the new confirmation. The known confirmation still moves to its new
    # parent chain block in the same run. This group holds the scenarios of both
    # results, and the scenario where the run writes nothing.
    #
    # Each test has two parts. The first part sets up the database to give
    # `:confirmation_missed`. The second part applies the indexer's change, and it
    ...
    describe "perform/5 with a new confirmation and a known one over an incomplete database" do
      ...
      # holds the parent chain block 190, but its event is in the block 200. The rollup
      # blocks 1..10 are also in the database, but no batch of the database holds them.
      #
      ...
      # It returns `:confirmation_missed` in the same run.
      #
      # In the second part of the test, the indexer writes the batch of the blocks
      # 1..10. Then the discovery examines the same parent chain range again. The
      # new confirmation covers the blocks 1..10. The known confirmation shows no
      # difference. The result is `:ok`.
      ...
      # The database has one batch of the blocks 11..20, and the known confirmation
      # holds those blocks. The known confirmation's parent chain block and its
      # timestamp are equal to the values in its event, which is in the block 200. The
      # rollup blocks 1..10 are also in the database, but no batch of the database
      # holds them.
      #
      ...
      # nothing, and it returns `:confirmation_missed`.
      #
      # The test "postpones the new confirmation and moves the known one to its new
      # parent chain block" uses the same database state. In that state, a re-org
      # already moved the known confirmation. In that other test, the run writes the
      # known transaction again. This test is the only one where a run of two events
      # writes nothing.
      #
      # In the second part of the test, the indexer writes the batch of the blocks
      # 1..10. Then the discovery examines the same parent chain range again. The new
      # confirmation covers the blocks 1..10, and the known confirmation keeps its
      # blocks and its values.
      ...
      # confirmation holds the full first batch. A re-org moved that confirmation: the
      # database holds the parent chain block 190, but its event is in the block 198.
      # The block 15 of the second batch is not linked to its batch. If the indexer did
      ...
      #
      # The test "postpones the new confirmation and moves the known one to its new
      # parent chain block" ends with the same pair of results. In that other test, an
      # unresolvable rollup block is the cause. This test is the only one where a gap of
      # a batch postpones the new confirmation and the known confirmation still moves.
      #
      # In the second part of the test, the indexer links the block 15 to its batch.
      # Then the discovery examines the same parent chain range again. The new
      # confirmation covers the blocks 11..20, and the walk stops at the first batch,
      # which is confirmed in full. The known confirmation shows no difference any
      # more.
