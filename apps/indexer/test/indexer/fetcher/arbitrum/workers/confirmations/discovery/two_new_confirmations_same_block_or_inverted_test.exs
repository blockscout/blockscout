...
if Application.get_env(:explorer, :chain_type) == :arbitrum do
  defmodule Indexer.Fetcher.Arbitrum.Workers.Confirmations.Discovery.TwoNewConfirmationsSameBlockOrInvertedTest do
    ...
    # of that confirmation. Thus the range holds no log of another confirmation of
    # the same parent chain block, and it holds no log of a confirmation of a newer
    # block.
    #
    ...
    # boundary which the other confirmation of the same run gives. Thus the two
    ...
    describe "perform/5 with two new confirmations in one parent chain block or in the inverted order" do
      ...
      # hash.
      #
      # This test is in the sets of two new confirmations because the parent chain
      # holds two new events. Its result holds one confirmation, and not two, because
      # the database keeps one confirmation per parent chain transaction.
      #
      # The database has one batch with the rollup blocks 1..20. No block of it is
      # confirmed. The lower event points to the rollup block 10. The upper event
      # points to the rollup block 20.
      ...
      # two logs by then. Thus the discovery examines no rollup block, and it keeps one
      # record with its blocks. This part is the only place which holds two logs of one
      # known transaction in one range. The first run must succeed before this part
      # runs, thus the part waits for the correction of the defect.
      ...
      #
      # Both events are new. The event of the older parent chain block points to the
      # rollup block 20. The event of the newer parent chain block points to the rollup
      # block 10. Thus the newer transaction confirms the lower rollup blocks. The HPP
      # mainnet, which is an Arbitrum AnyTrust chain, holds such a pair of
      # confirmations.
      #
      # The confirmation of the block 10 must cover the blocks 1..10, and the
      # confirmation of the block 20 must cover the blocks 11..20.
      ...
      # Thus it handles the confirmation of the block 10 first. The lookup range of
      # that confirmation holds the log of the confirmation of the block 20. That log
      # points to a block of the same batch, thus the discovery takes the block 21 as
      # the first unconfirmed block of the batch. The block 21 is above the block 10.
      # Therefore the confirmation of the block 10 gets no rollup block.
      #
      # After that the confirmation of the block 20 finds no earlier log in its own
      # lookup range, and it takes the full batch. Thus the run writes one confirmation
      # of the two, and it returns `:confirmation_missed`.
      #
      # The test "splits the batches between the two confirmations of the inverted
      # order" holds the same pair of events over two batches. There the import stops
      # with a cardinality violation, because the two confirmations hold the same
      # blocks. This test is the only one where the inverted order gives one
      # confirmation of the whole batch and loses the other one.
      #
      ...
      # repeated run of the same parent chain range must give `:ok` and the same pair
      # of confirmations. The discovery gives `:confirmation_missed` again, because the
      # full batch belongs to the confirmation of the block 20 now. Thus the historical
      # discovery reads this range again and again.
      #
      # The correction of the defect gives the correct pair in the first run. Thus the
