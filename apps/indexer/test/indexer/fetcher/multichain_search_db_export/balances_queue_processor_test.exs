defmodule Indexer.Fetcher.MultichainSearchDbExport.BalancesQueueProcessorTest do
  use ExUnit.Case
  ...
  alias Indexer.Fetcher.MultichainSearchDbExport.BalancesQueueProcessor,
    as: MultichainSearchDbExportBalancesQueueProcessor
  ...
  setup do
    ...
    Application.put_env(:indexer, MultichainSearchDbExportBalancesQueueProcessor.Supervisor, disabled?: false)

    on_exit(fn ->
      Application.put_env(:indexer, MultichainSearchDbExportBalancesQueueProcessor.Supervisor, disabled?: true)
    end)
  ...
  describe "init/3" do
    ...
    test "initializes with data from the retry queue" do
      ...
      pid =
        []
        |> MultichainSearchDbExportBalancesQueueProcessor.Supervisor.child_spec()
        |> ExUnit.Callbacks.start_supervised!()

      results = MultichainSearchDbExportBalancesQueueProcessor.init([], reducer, nil)
  ...
  describe "run/2" do
    ...
    test "successfully processes multichain search db export retry queue data", %{bypass: bypass} do
      ...
      assert :ok = MultichainSearchDbExportBalancesQueueProcessor.run(export_data, nil)
    end
      ...
      log =
        capture_log(fn ->
          assert {:retry,
                  ...
                  }} = MultichainSearchDbExportBalancesQueueProcessor.run(export_data, nil)
        end)
