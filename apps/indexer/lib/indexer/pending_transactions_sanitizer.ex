defmodule Indexer.PendingTransactionsSanitizer do
  ...
  defp fetch_block_and_invalidate(block_hash, pending_transaction, tx) do
    case Chain.fetch_block_by_hash(block_hash) do
      %{number: number, consensus: consensus} = block ->
        ...
        invalidate_block(block, pending_transaction, tx)
  ...
  defp invalidate_block(block, pending_transaction, tx) do
    if block.consensus do
      ...
      Logger.debug(
        "Pending tx with hash #{"0x" <> Base.encode16(pending_transaction.hash.bytes, case: :lower)} assigned to block ##{block.number} with hash #{block.hash}"
      )
    end
  end
end
