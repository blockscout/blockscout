defmodule Indexer.Block.Fetcher do
  ...
  defp process_massive_blocks(fetched_blocks) do
    massive_block_threshold = Application.get_env(:indexer, :massive_block_threshold)

    massive_block_numbers =
      fetched_blocks
      |> Map.get(:transactions_params, [])
      |> Enum.group_by(& &1.block_number)
      |> Enum.reduce([], fn {number, transactions}, acc ->
        if Enum.count(transactions) > massive_block_threshold, do: [number | acc], else: acc
      end)
