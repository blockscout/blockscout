  ...
  def init(initial, reducer, json_rpc_named_arguments) do
    stream_reducer =
      if RangesHelper.trace_ranges_present?() do
        ...
        fn block_number_or_transaction_params, acc ->
          block_number =
            case block_number_or_transaction_params do
          ...
          if RangesHelper.number_in_ranges?(block_number, trace_block_ranges),
            do: reducer.(block_number_or_transaction_params, acc),
            else: acc
        end
      else
        fn block_number_or_transaction_params, acc ->
          reducer.(block_number_or_transaction_params, acc)
        end
      end
