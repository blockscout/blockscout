...
defmodule Indexer.Fetcher.OnDemand.InternalTransaction do
  ...
  @spec fetch_by_address(Hash.Address.t(), Keyword.t(), integer()) :: [InternalTransaction.t()]
  def fetch_by_address(_address_hash, _options, limit) when limit <= 0, do: []

  def fetch_by_address(address_hash, options, limit) do
    necessity_by_association = Keyword.get(options, :necessity_by_association, %{})
    ...
    address_id
    |> get_block_numbers_for_address(max_block_number, limit, sum_mode)
    |> Enum.reject(&(&1 < (from_block || 0)))
    |> fetch_block_internal_transactions()
    |> Enum.map(&serialize/1)
  ...
  defp get_block_numbers_for_address(nil, _start_block, _limit, _sum_mode), do: []

  defp get_block_numbers_for_address(address_id, start_block, limit, sum_mode) do
    ranked_query =
      ...
      |> windows([q], w: [partition_by: :address_id, order_by: [desc: :block_number]])
      |> select([q], %{
  ...
  defp fetch_block_internal_transactions(block_numbers) do
    ...
    if variant in InternalTransactionFetcher.block_traceable_variants() do
      ...
      Enum.reduce(block_numbers, [], fn block_number, acc_list ->
        ...
        |> Enum.map(&%{hash_data: to_string(&1.hash)})
        |> case do
          [] ->
            {:ok, []}
  ...
  def join_associations(records, necessity_by_association)
  ...
  def join_association(records, [{association, nested_preload}], :optional)
      when is_atom(association) do
    Repo.preload(records, [{association, nested_preload}])
  end

  def join_association(records, [{association, nested_preload}], :required)
      when is_atom(association) do
    records
  ...
  def join_association(records, association, :optional) do
    Repo.preload(records, association)
  end

  def join_association(records, association, :required) do
    records
