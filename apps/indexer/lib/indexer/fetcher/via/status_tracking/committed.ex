defmodule Indexer.Fetcher.Via.StatusTracking.Committed do
  @moduledoc """
    Functionality to discover committed batches
  """

  alias Indexer.Fetcher.Via.Utils.{Db, Rpc}

  import Indexer.Fetcher.Via.StatusTracking.CommonUtils,
    only: [
      check_if_batch_status_changed: 3,
      associate_and_import_or_prepare_for_recovery: 4
    ]

  import Indexer.Fetcher.Via.Utils.Logging, only: [log_info: 1]
      ...
      expected_batch_number ->
        ...
        case next_action do
          ...
          :look_for_batches ->
            ...
            # Todo: Should we fetch the inscription from BTC chain or only L2?
