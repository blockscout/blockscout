defmodule Indexer.Fetcher.Via.TransactionBatch do
  @moduledoc """
    Discovers new batches and populates the `via_transaction_batches` table.
    ...
    in chunks of `chunk_size` and imported into the `via_transaction_batches` table. If the latest
  ...
  alias Indexer.Fetcher.Via.Utils.Rpc
  ...
  @impl GenServer
  def init(args) do
    Logger.metadata(fetcher: :via_transaction_batches)

    config = Application.get_all_env(:indexer)[Indexer.Fetcher.Via.TransactionBatch]
