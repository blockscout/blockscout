defmodule Indexer.Fetcher.Via.Utils.Db do
  @moduledoc """
    Common functions to simplify DB routines for Indexer.Fetcher.Via fetchers
  """

  alias Explorer.Chain
  alias Explorer.Chain.Via.Reader
  import Indexer.Fetcher.Via.Utils.Logging, only: [log_warning: 1, log_info: 1]
  ...
  @doc """
    ...
    - `l1_transactions`: A map of L1 transaction descriptions. Each element is extended with
      ...
      `via_lifecycle_l1_transactions` table.
  """
  ...
  def import_to_db(batches, l1_transactions \\ [], l2_transactions \\ [], l2_blocks \\ [])
    ...
    {:ok, _} =
      Chain.import(%{
        via_lifecycle_transactions: %{params: l1_transactions},
        via_transaction_batches: %{params: batches},
        via_batch_transactions: %{params: l2_transactions},
        via_batch_blocks: %{params: l2_blocks},
        timeout: :infinity
      })
  end
end
