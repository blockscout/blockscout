defmodule Indexer.Fetcher.Via.BatchesStatusTracker do
  @moduledoc """
    Updates batches statuses and imports historical batches to the `via_transaction_batches` table.
    ...
    in the `via_transaction_batches` table is not possible because some are missing,
    ...
    to be done by `Indexer.Fetcher.Via.TransactionBatch` or during handling of
  ...
  alias Indexer.Fetcher.Via.Discovery.Workers
  alias Indexer.Fetcher.Via.StatusTracking.{Committed, Executed, Proven}
  ...
  @impl GenServer
  def init(args) do
    Logger.metadata(fetcher: :via_batches_tracker)

    config_tracker = Application.get_all_env(:indexer)[Indexer.Fetcher.Via.BatchesStatusTracker]
    l1_rpc = config_tracker[:via_l1_rpc]
    recheck_interval = config_tracker[:recheck_interval]
    config_fetcher = Application.get_all_env(:indexer)[Indexer.Fetcher.Via.TransactionBatch]
  ...
  # import them to the `via_transaction_batches` table. The batches are imported together with L1
  ...
  # to the `via_transaction_batches` table. It is expected that the message is sent from handlers updating
  # batches statuses when they discover the absence of batches in the `via_transaction_batches` table.
