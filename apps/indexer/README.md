...
- `coin_balances_delta`: computes the delta of existing `address_coin_balances` with a value and saves the result in the database. This is done as part of [PR #1991](https://github.com/poanetwork/blockscout/pull/1991), where `delta` has been added to the schema, to avoid having a long-running migration.
