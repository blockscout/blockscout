...
/*
 * Crypto stamp 3.1 On-Chain Shop
 * Ability to purchase digital-physical collectible postage stamps and to
 * redeem Crypto stamp 3.1 pre-sale vouchers in a similar manner
 *
...
// File: contracts/CS3PresaleRedeemI.sol

/*
 * Interface for CS3 on-chain presale for usage with redeemer (OCS) contract.
 */

interface CS3PresaleRedeemI is IERC1155 {

    // Redeem assets of potentially multiple types/animals at once.
    // This burns them in this contract, but should be called by a contract that assigns/creates the final assets in turn.
    function redeemBatch(address owner, uint256[] calldata _type, uint256[] calldata _count) external;
}
...
contract CS3_1OnChainShop {
    ...
    IERC721Enumerable internal CS3_1;
    CS3PresaleRedeemI internal CS3_1Presale;
    ...
    address[4] public tokenPools; // Pool 0 and 1 are "normal" OCS, 2 and 3 are presale.
    ...
    event ShopClosed();
    event AssetSold(address indexed buyer, address recipient, bool presale, CS3_1PropertiesI.AssetType indexed aType, uint256 indexed tokenId, uint256 priceWei);

    constructor(address _bridgeDataAddress, address _shippingManagerAddress, address _CS3_1Address, address _CS3_1PresaleAddress, uint256 _priceEurCent, address[] memory _tokenPools)
    {
        ...
        CS3_1 = IERC721Enumerable(_CS3_1Address);
        require(address(CS3_1) != address(0x0), "You need to provide an actual Cryptostamp 3.1 contract.");
        CS3_1Presale = CS3PresaleRedeemI(_CS3_1PresaleAddress);
        require(address(CS3_1Presale) != address(0x0), "You need to provide an actual Cryptostamp 3.1 Presale contract.");
    ...
    function getPool(bool _presale, CS3_1PropertiesI.AssetType _type)
    public view
    returns (address)
    {
        return tokenPools[(_presale ? 2 : 0) + uint256(_type)];
    }

    // Returns the amount of assets that are still available for sale.
    function availableForSale(bool _presale, CS3_1PropertiesI.AssetType _type)
    public view
    returns (uint256)
    {
        return CS3_1.balanceOf(getPool(_presale, _type));
    }

    // Returns true if the asset of the given type is sold out.
    function isSoldOut(bool _presale, CS3_1PropertiesI.AssetType _type)
    public view
    returns (bool)
    {
    ...
    requireOpen
    {
        ...
        address pool = getPool(isPresale, _type);
        uint256 availAmount = CS3_1.balanceOf(pool);
        ...
        transferStamps(_recipient, _type, assetCount, isPresale, pool, availAmount, curPriceWei);
    ...
    function redeemVoucher(CS3_1PropertiesI.AssetType _type, uint256 _amount, address _recipient, bool _acceptTermsCryptoPostAt, string calldata _acceptanceText)
    public
    requireOpen
    {
        require(_acceptTermsCryptoPostAt, "You need to accept the terms.");
        require(bytes(_acceptanceText).length > 0, "You need to send the acceptance text.");
        bool isPresale = true;
        uint256 CS3_1PresaleId = uint256(_type);
        address pool = getPool(isPresale, _type);
        uint256 availAmount = CS3_1.balanceOf(pool);
        require(_amount <= availAmount, "Not enough assets available to buy that amount.");
        require(CS3_1Presale.balanceOf(msg.sender, CS3_1PresaleId) >= _amount, "You need to own enough presale vouchers to redeem the specified amount.");
        // Redeem the vouchers.
        uint256[] memory redeemTypes = new uint256[](1);
        uint256[] memory redeemAmounts = new uint256[](1);
        redeemTypes[0] = CS3_1PresaleId;
        redeemAmounts[0] = _amount;
        CS3_1Presale.redeemBatch(msg.sender, redeemTypes, redeemAmounts);
        transferStamps(_recipient, _type, _amount, isPresale, pool, availAmount, 0);
    }

    function transferStamps(address _recipient, CS3_1PropertiesI.AssetType _type, uint256 _amount, bool _presale, address _pool, uint256 _availAmount, uint256 _priceWei)
    internal
    {
        for (uint256 i = 0; i < _amount; i++) {
            // Find the next stamp (last owned by pool) and transfer it.
            uint256 tokenId = CS3_1.tokenOfOwnerByIndex(_pool, _availAmount - i - 1);
            CS3_1.safeTransferFrom(_pool, _recipient, tokenId);
            emit AssetSold(msg.sender, _recipient, _presale, _type, tokenId, _priceWei);
            shippingManager.setShippingStatus(address(CS3_1), tokenId, ShippingManagerI.ShippingStatus.Sold);
        }
    }
