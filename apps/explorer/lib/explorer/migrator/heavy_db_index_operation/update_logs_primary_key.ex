...
defmodule Explorer.Migrator.HeavyDbIndexOperation.UpdateLogsPrimaryKey do
  @moduledoc """
  Update primary key on `logs` table from (`block_hash`, `transaction_hash`, `index`) to (`block_number`, `transaction_index`, `index`).
  """
  ...
  alias Explorer.Migrator.HeavyDbIndexOperation.{
    ...
    ValidateLogsBlockNumberTransactionIndexNotNull
  }
  ...
  def dependent_from_migrations,
    do: [
      ValidateLogsBlockNumberTransactionIndexNotNull.migration_name()
    ]
  ...
  def db_index_operation do
    result =
      Repo.transaction(fn ->
        with {:ok, _} <- Repo.query(drop_pk_constraint_query_string()),
             ...
             {:ok, _} <- Repo.query(drop_transaction_hash_not_null_query_string()) do
          update_cache()
          :ok
        else
          {:error, error} ->
    ...
    case result do
      ...
      {:error, error} ->
        Logger.error("Migration UpdateLogsPrimaryKey failed: #{inspect(error)}")
        :error
    end
  end
  ...
  def check_db_index_operation_progress do
    all_operations = [
      ...
      drop_transaction_hash_not_null_query_string()
    ]
  ...
  def restart_db_index_operation do
    result =
      Repo.transaction(fn ->
        with {:ok, _} <- Repo.query(drop_pk_constraint_query_string()),
             ...
             {:ok, _} <- Repo.query(set_transaction_hash_not_null_query_string()),
    ...
    case result do
      ...
      {:error, error} ->
        Logger.error("Migration UpdateLogsPrimaryKey rollback failed: #{inspect(error)}")
        :error
    end
  end
  ...
  @impl HeavyDbIndexOperation
  def update_cache do
    BackgroundMigrations.set_heavy_indexes_update_logs_primary_key_finished(true)
  end
  ...
  defp rename_index_query_string do
    "ALTER INDEX #{CreateLogsBlockNumberTransactionIndexIndexUniqueIndex.index_name()} RENAME TO #{@index_name};"
  end
  ...
  defp drop_transaction_hash_not_null_query_string do
    "ALTER TABLE #{@table_name} ALTER COLUMN transaction_hash DROP NOT NULL;"
  end

  defp set_transaction_hash_not_null_query_string do
    "ALTER TABLE #{@table_name} ALTER COLUMN transaction_hash SET NOT NULL;"
  end
