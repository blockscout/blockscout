...
defmodule Explorer.Migrator.BackfillScaledUIAmountTokens do
  ...
  def last_unprocessed_identifiers(state) do
    ...
    ids =
      ...
      |> select([log], {log.block_number, log.index})
      |> limit(^limit)
      |> Repo.all(timeout: :infinity)

    {ids, advance(state, ids)}
  end

  @impl FillingMigration
  def unprocessed_data_query do
    topic_filter = Log.topic_filter_dynamic(:first_topic, [ScaledUIAmount.ui_multiplier_updated_event()])

    from(log in Log, as: :log, where: ^topic_filter)
  end

  defp advance(state, []), do: state
  defp advance(state, ids), do: Map.put(state, "last_position", ids |> List.last() |> Tuple.to_list())
  ...
  @impl FillingMigration
  def update_batch(log_ids) do
    wanted = MapSet.new(log_ids)

    changes =
      log_ids
      |> logs_query()
      |> Repo.all(timeout: :infinity)
      |> Enum.filter(&MapSet.member?(wanted, {&1.block_number, &1.index}))
      |> Log.prepare_data()
      |> Enum.map(&with_address_hash/1)
      |> Enum.reject(&is_nil(&1.address_hash))
      |> Enum.map(&ScaledUIAmount.parse_known_ui_multiplier_updated/1)
    ...
    verified_hashes =
      ...
      |> Enum.uniq()
      |> Enum.filter(&scaled_ui_amount_token?/1)

    verified = MapSet.new(verified_hashes)

    changes
    |> Enum.filter(&MapSet.member?(verified, &1.token_contract_address_hash))
    |> UIMultiplierChange.insert_changes()

    type_as_scaled_ui_amount(verified_hashes)

    length(log_ids)
  end

  defp scaled_ui_amount_token?(contract_address_hash) do
    contract_address_hash |> Hash.to_string() |> MetadataRetriever.supports_scaled_ui_amount?()
  end

  @impl FillingMigration
  def update_cache, do: :ok

  defp logs_query(log_ids) do
    {block_numbers, indexes} = Enum.unzip(log_ids)

    from(log in Log,
      preload: [:address_mapping],
      where: log.block_number in ^Enum.uniq(block_numbers),
      where: log.index in ^Enum.uniq(indexes)
    )
  end

  defp with_address_hash(%Log{address_hash: nil, address_mapping: %AddressIdToAddressHash{} = mapping} = log),
    do: %{log | address_hash: mapping.address_hash}

  defp with_address_hash(log), do: log
