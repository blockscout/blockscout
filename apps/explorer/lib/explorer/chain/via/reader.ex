defmodule Explorer.Chain.Via.Reader do
  @moduledoc "Contains read functions for via modules."
  ...
  @doc """
    Receives total amount of batches imported to the `via_transaction_batches` table.
  ...
  @doc """
    Receives the batch from the `via_transaction_batches` table by using its number or the latest batch if `:latest` is used.
    ...
    ## Returns
    - `{:ok, Explorer.Chain.Via.TransactionBatch}` if the batch found
    - `{:error, :not_found}` if there is no batch with such number
  """
  @spec batch(:latest | binary() | integer(), keyword()) ::
          {:error, :not_found} | {:ok, Explorer.Chain.Via.TransactionBatch}
  ...
  @doc """
    Receives a list of batches from the `via_transaction_batches` table within the range of batch numbers
    ...
    ## Returns
    - A list of `Explorer.Chain.Via.TransactionBatch` if at least one batch exists within the range.
    - An empty list (`[]`) if no batches within the range are found in the database.
  """
  @spec batches(integer(), integer(), keyword()) :: [Explorer.Chain.Via.TransactionBatch]
  ...
  @doc """
    Receives a list of batches from the `via_transaction_batches` table with the numbers defined in the input list.
    ...
    ## Returns
    - A list of `Explorer.Chain.Via.TransactionBatch` if at least one batch matches the numbers from the list. The output list could be less than the input list.
    - An empty list (`[]`) if no batches with numbers from the list are found.
  """
  @spec batches(maybe_improper_list(integer(), []), keyword()) :: [Explorer.Chain.Via.TransactionBatch]
  ...
  @doc """
    Receives a list of batches from the `via_transaction_batches` table.

    ## Parameters
    - `options`: Options passed to `Chain.select_repo()`. (Optional)

    ## Returns
    - If the option `confirmed?` is set, returns the ten latest committed batches (`Explorer.Chain.Via.TransactionBatch`).
    - Returns a list of `Explorer.Chain.Via.TransactionBatch` based on the paging options if `confirmed?` is not set.
  """
  @spec batches(keyword()) :: [Explorer.Chain.Via.TransactionBatch]
  @spec batches() :: [Explorer.Chain.Via.TransactionBatch]
  ...
  @doc """
    Receives a list of transactions from the `via_batch_l2_transactions` table included in a specific batch.
    ...
    ## Returns
    - A list of `Explorer.Chain.Via.BatchTransaction` belonging to the specified batch.
  """
  @spec batch_transactions(non_neg_integer()) :: [Explorer.Chain.Via.BatchTransaction]
  @spec batch_transactions(non_neg_integer(), keyword()) :: [Explorer.Chain.Via.BatchTransaction]
  ...
  @doc """
    Gets the number of the earliest batch in the `via_transaction_batches` table where the commitment transaction is not set.
  ...
  @doc """
    Gets the number of the earliest batch in the `via_transaction_batches` table where the proving transaction is not set.
  ...
  @doc """
    Gets the number of the earliest batch in the `via_transaction_batches` table where the executing transaction is not set.
  ...
  @doc """
    Gets the number of the oldest batch from the `via_transaction_batches` table.
  ...
  @doc """
    Gets the number of the youngest (the most recent) imported batch from the `via_transaction_batches` table.
  ...
  @doc """
    Reads a list of L1 transactions by their hashes from the `via_lifecycle_l1_transactions` table.

    ## Parameters
    - `l1_transaction_hashes`: A list of hashes to retrieve L1 transactions for.

    ## Returns
    - A list of `Explorer.Chain.Via.LifecycleTransaction` corresponding to the hashes from the input list. The output list may be smaller than the input list.
  """
  @spec lifecycle_transactions(maybe_improper_list(binary(), [])) :: [Explorer.Chain.Via.LifecycleTransaction]
  ...
  @doc """
    Determines the next index for the L1 transaction available in the `via_lifecycle_l1_transactions` table.
