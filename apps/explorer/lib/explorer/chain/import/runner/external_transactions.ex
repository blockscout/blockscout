defmodule Explorer.Chain.Import.Runner.ExternalTransactions do
  @moduledoc """
  ...
  alias Explorer.Chain.{Block, Hash, Import, ExternalTransaction}
  ...
  def run(multi, changes_list, %{timestamps: timestamps} = options) do
    insert_options =
      ...
      |> Map.put(:token_transfer_external_transaction_hash_set, token_transfer_external_transaction_hash_set(options))

    # Enforce ShareLocks tables order (see docs: sharelocks.md)
    multi
    |> Multi.run(:recollated_external_transactions, fn repo, _ ->
      Instrumenter.block_import_stage_runner(
        fn ->
          discard_blocks_for_recollated_external_transactions(repo, changes_list, insert_options)
        end,
        :block_referencing,
        :transactions,
    ...
    |> Multi.run(:external_transactions, fn repo, _ ->
      Instrumenter.block_import_stage_runner(
  ...
  defp token_transfer_external_transaction_hash_set(options) do
  ...
  defp discard_blocks_for_recollated_external_transactions(repo, changes_list, %{
         timeout: timeout,
