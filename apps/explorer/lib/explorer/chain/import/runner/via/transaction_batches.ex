defmodule Explorer.Chain.Import.Runner.Via.TransactionBatches do
  @moduledoc """
  Bulk imports `t:Explorer.Chain.Via.TransactionBatch.t/0`.
  """
  ...
  @impl Import.Runner
  def option_key, do: :via_transaction_batches
  ...
  def run(multi, changes_list, %{timestamps: timestamps} = options) do
    ...
    Multi.run(multi, :insert_via_transaction_batches, fn repo, _ ->
      Instrumenter.block_import_stage_runner(
        fn -> insert(repo, changes_list, insert_options) end,
        :block_referencing,
        :via_transaction_batches,
        :via_transaction_batches
      )
    end)
  end
  ...
  def insert(repo, changes_list, %{timeout: timeout, timestamps: timestamps} = options) when is_list(changes_list) do
    ...
    # Enforce Via.TransactionBatch ShareLocks order (see docs: sharelock.md)
