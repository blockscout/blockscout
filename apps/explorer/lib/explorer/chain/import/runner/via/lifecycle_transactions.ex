defmodule Explorer.Chain.Import.Runner.Via.LifecycleTransactions do
  @moduledoc """
  Bulk imports `t:Explorer.Chain.Via.LifecycleTransaction.t/0`.
  """
  ...
  alias Explorer.Chain.Via.LifecycleTransaction
  ...
  @impl Import.Runner
  def option_key, do: :via_lifecycle_transactions
  ...
  def run(multi, changes_list, %{timestamps: timestamps} = options) do
    ...
    Multi.run(multi, :insert_via_lifecycle_transactions, fn repo, _ ->
      Instrumenter.block_import_stage_runner(
        fn -> insert(repo, changes_list, insert_options) end,
        :block_referencing,
        :via_lifecycle_transactions,
        :via_lifecycle_transactions
      )
    end)
  end
  ...
  def insert(repo, changes_list, %{timeout: timeout, timestamps: timestamps} = options) when is_list(changes_list) do
    ...
    # Enforce Via.LifecycleTransaction ShareLocks order (see docs: sharelock.md)
