defmodule Explorer.Chain.Import.Runner.Zilliqa.Zrc2.TokenAdapters do
  @moduledoc """
  Bulk imports `t:Explorer.Chain.Zilliqa.Zrc2.TokenAdapter.t/0`.
  """
  ...
  def insert(repo, changes_list, %{timeout: timeout, timestamps: timestamps} = _options) when is_list(changes_list) do
    # Enforce TokenAdapter ShareLocks order (see docs: sharelock.md)
    ordered_changes_list = Enum.sort_by(changes_list, & &1.adapter_address_hash)
    ...
    {:ok, inserted} =
      Import.insert_changes_list(
        ...
        conflict_target: :adapter_address_hash,
        on_conflict: :nothing
      )
