defmodule Explorer.Chain.Address.ScamBadgeToAddress do
  ...
  * `address_hash` - foreign key for `address`.
  ...
  def add(address_hashes) do
    ...
    # if at least one of the address hashes didn't yet exist in the `addresses` table we'll receive foreign key violation error
    error ->
      with %Postgrex.Error{postgres: %{code: :foreign_key_violation}} <- error,
           {_number_of_inserted_addresses, [_term]} <- Address.create_multiple(address_hashes) do
        Address.create_multiple(address_hashes)
        add(address_hashes)
      else
        _ -> {0, []}
      end
