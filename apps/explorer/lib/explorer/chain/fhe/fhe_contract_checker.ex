defmodule Explorer.Chain.FheContractChecker do
  ...
  def check_and_save_fhe_status(address_hash, options) when not is_nil(address_hash) do
    ...
    cond do
      not Address.smart_contract?(address) or is_nil(address.contract_code) -> :empty
      already_checked?(address_hash, options) -> :already_checked
      true -> do_check_and_save_fhe_status(address_hash, options)
    end
  end

  def check_and_save_fhe_status(_, _), do: :empty

  defp do_check_and_save_fhe_status(address_hash, options) do
  ...
  """
  @spec fhe_contract?(Hash.Address.t()) :: {:ok, boolean()}
  def fhe_contract?(%Hash{byte_count: 20} = address_hash) do
  ...
  @spec fhe_contract?(binary()) :: {:ok, boolean()} | {:error, :invalid_hash}
  def fhe_contract?(address_hash_string) when is_binary(address_hash_string) do
    case Hash.Address.cast(address_hash_string) do
      {:ok, address_hash} ->
        fhe_contract?(address_hash)
  ...
  defp ensure_fhe_tag_exists do
    case AddressTag.set(@fhe_tag_label, @fhe_tag_display_name) do
      ...
      {:error,
       %Ecto.Changeset{errors: [label: {_, [constraint: :unique, constraint_name: "address_tags_label_index"]}]}} ->
        :ok
