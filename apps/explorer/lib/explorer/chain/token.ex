defmodule Explorer.Chain.Token do
  ...
  * `icon_url` - URL of the token's icon.
  ...
  defp apply_filter(query, empty_filter) when empty_filter in [nil, []], do: query

  defp apply_filter(query, filter) when is_list(filter) do
    from(t in query, where: t.cataloged == ^filter)
  end
