defmodule Explorer.Chain.ExternalTransaction.CallType do
  @moduledoc """
  ...
  If the `term` is already in `t:t/0`, then it is returned
      ...
      iex> Explorer.Chain.ExternalTransaction.CallType.cast(:call)
      {:ok, :call}
      iex> Explorer.Chain.ExternalTransaction.CallType.cast(:callcode)
      {:ok, :callcode}
      iex> Explorer.Chain.ExternalTransaction.CallType.cast(:delegatecall)
      {:ok, :delegatecall}
      iex> Explorer.Chain.ExternalTransaction.CallType.cast(:staticcall)
      {:ok, :staticcall}
  ...
  Unsupported `String.t` return an `:error`.
      ...
      iex> Explorer.Chain.ExternalTransaction.CallType.cast("hard-fork")
      :error
  ...
  Dumps the `atom` format to `String.t` format used in the database.
      ...
      iex> Explorer.Chain.ExternalTransaction.CallType.dump(:call)
      {:ok, "call"}
      iex> Explorer.Chain.ExternalTransaction.CallType.dump(:callcode)
      {:ok, "callcode"}
      iex> Explorer.Chain.ExternalTransaction.CallType.dump(:delegatecall)
      {:ok, "delegatecall"}
      iex> Explorer.Chain.ExternalTransaction.CallType.dump(:staticcall)
      {:ok, "staticcall"}

  Other atoms return an error

      iex> Explorer.Chain.ExternalTransaction.CallType.dump(:other)
      :error
  ...
  Loads the `t:String.t/0` from the database.
      ...
      iex> Explorer.Chain.ExternalTransaction.CallType.load("call")
      {:ok, :call}
      iex> Explorer.Chain.ExternalTransaction.CallType.load("callcode")
      {:ok, :callcode}
      iex> Explorer.Chain.ExternalTransaction.CallType.load("delegatecall")
      {:ok, :delegatecall}
      iex> Explorer.Chain.ExternalTransaction.CallType.load("staticcall")
      {:ok, :staticcall}

  Other `t:String.t/0` return `:error`

      iex> Explorer.Chain.ExternalTransaction.CallType.load("other")
      :error
