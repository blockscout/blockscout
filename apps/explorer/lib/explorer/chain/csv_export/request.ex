defmodule Explorer.Chain.CsvExport.Request do
  ...
  def create(remote_ip, args) do
    ...
    Repo.transact(fn ->
      ...
      with {:too_many_pending_requests, false} <- {:too_many_pending_requests, pending_count >= max_pending},
           {:ok, request} <-
        ...
        {:too_many_pending_requests, true} -> Repo.rollback(:too_many_pending_requests)
