defmodule Explorer.Chain.SmartContract.Proxy.ResolvedDelegateProxy do
  ...
  def get_prefetch_requirements(proxy_address, _) do
    # slot 0
  ...
  @doc """
  Get implementation address hash following Legacy OP-stack ResolvedDelegateProxy proxy pattern.
  """
  ...
  def match_bytecode_and_resolve_implementation(proxy_address) do
    with false <- is_nil(proxy_address.contract_code),
         @resolved_delegate_proxy <- proxy_address.contract_code.bytes,
         reqs = get_prefetch_requirements(proxy_address, :resolved_delegate_proxy),
