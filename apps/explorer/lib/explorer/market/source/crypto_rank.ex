defmodule Explorer.Market.Source.CryptoRank do
  ...
  def fetch_native_coin do
    do_fetch_coin(config(:coin_id), "Coin ID not specified")
  end
  ...
  @impl Source
  def fetch_secondary_coin do
    do_fetch_coin(config(:secondary_coin_id), "Secondary coin ID not specified")
  end

  @impl Source
  def tokens_fetching_enabled? do
    not is_nil(config(:platform))
  end
  ...
  @impl Source
  def fetch_native_coin_price_history(previous_days) do
    do_fetch_coin_price_history(previous_days, false)
  end
  ...
  @impl Source
  def fetch_secondary_coin_price_history(previous_days) do
    do_fetch_coin_price_history(previous_days, true)
  end
  ...
  defp do_fetch_coin(nil, coin_id_not_specified_error) do
    {:error, coin_id_not_specified_error}
  end

  defp do_fetch_coin(coin_id, _coin_id_not_specified_error) do
    case Source.http_request(base_url() |> URI.append_path("/currencies/#{coin_id}") |> URI.to_string(), headers()) do
      {:ok, data} ->
        coin = data["data"]
  ...
  defp do_fetch_coin_price_history(previous_days, secondary_coin?) do
    with coin_id when not is_nil(coin_id) <-
      ...
      [_ | closing_prices] = opening_prices
      ...
      nil -> {:error, "#{if secondary_coin?, do: "Secondary c", else: "C"}oin ID not specified"}
      {:ok, nil} -> {:ok, []}
