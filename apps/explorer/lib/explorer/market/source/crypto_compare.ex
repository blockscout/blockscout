defmodule Explorer.Market.Source.CryptoCompare do
  ...
  defp do_fetch_coin_price_history(previous_days, secondary_coin?) do
    with coin_id when not is_nil(coin_id) <-
           if(secondary_coin?, do: config(:secondary_coin_symbol), else: Explorer.coin()),
         {:ok, %{"Data" => %{"Data" => data}}} <-
           Source.http_request(
             :base_url
             |> config()
      ...
      {:ok, nil} -> {:ok, []}
      {:ok, unexpected_response} -> {:ok, "Unexpected response from CryptoCompare: #{inspect(unexpected_response)}"}
