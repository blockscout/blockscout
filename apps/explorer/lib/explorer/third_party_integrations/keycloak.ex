defmodule Explorer.ThirdPartyIntegrations.Keycloak do
  ...
  alias Explorer.{HttpClient, Mailer, Vault}
  ...
  defp find_users_by_address(address) do
  ...
  defp otp_key(email), do: "#{client_id()}:otp:#{String.downcase(email)}"
  defp otp_attempts_key(email), do: "#{client_id()}:otp_attempts:#{String.downcase(email)}"
