defmodule BlockScoutWeb.ExternalTransactionView do
  ...
  alias Explorer.Chain.{Address, Block, InternalTransaction, Transaction, ExternalTransaction, Wei}
  ...
  defguardp is_transaction_type(mod) when mod in [InternalTransaction, Transaction, ExternalTransaction]

  defdelegate formatted_timestamp(block), to: BlockView

  def block_number(%ExternalTransaction{block_number: nil}), do: gettext("Block Pending")
  def block_number(%ExternalTransaction{block: block}), do: [view_module: BlockView, partial: "_link.html", block: block]
  def block_number(%Reward{block: block}), do: [view_module: BlockView, partial: "_link.html", block: block]

  def block_timestamp(%ExternalTransaction{block_number: nil, inserted_at: time}), do: time
  def block_timestamp(%ExternalTransaction{block: %Block{timestamp: time}}), do: time
  def block_timestamp(%Reward{block: %Block{timestamp: time}}), do: time

  def value_transfer?(%ExternalTransaction{input: %{bytes: bytes}}) when bytes in [<<>>, nil] do
    true
  end
  ...
  def processing_time_duration(%ExternalTransaction{block: nil}) do
    :pending
  end

  def processing_time_duration(%ExternalTransaction{earliest_processing_start: nil}) do
  ...
  def processing_time_duration(%ExternalTransaction{
  ...
  def confirmations(%ExternalTransaction{block: block}, named_arguments) when is_list(named_arguments) do
    case block do
  ...
  def contract_creation?(%ExternalTransaction{to_address: nil}), do: true

  def contract_creation?(_), do: false

  def fee(%ExternalTransaction{} = transaction) do
  ...
  def formatted_fee(%ExternalTransaction{} = transaction, opts) do
    transaction
    |> Chain.fee(:wei)
  ...
  def decoded_input_data(transaction) do
    ExternalTransaction.decoded_input_data(transaction)
  end

  def decoded_revert_reason(revert_reason, transaction) do
    ExternalTransaction.decoded_revert_reason(transaction, revert_reason)
  end

  @doc """
  Converts a transaction's gas price to a displayable value.
  """
  def gas_price(%ExternalTransaction{gas_price: gas_price}, unit) when unit in ~w(wei gwei ether)a do
    format_wei_value(gas_price, unit)
  end

  def gas_used(%ExternalTransaction{gas_used: nil}), do: gettext("Pending")

  def gas_used(%ExternalTransaction{gas_used: gas_used}) do
    Number.to_string!(gas_used)
  end

  def gas_used_perc(%ExternalTransaction{gas_used: nil}), do: nil

  def gas_used_perc(%ExternalTransaction{gas_used: gas_used, gas: gas}) do
  ...
  def hash(%ExternalTransaction{hash: hash}) do
    to_string(hash)
  end

  def involves_contract?(%ExternalTransaction{from_address: from_address, to_address: to_address}) do
    AddressView.contract?(from_address) || AddressView.contract?(to_address)
  end

  def involves_token_transfers?(_), do: false

  def qr_code(%ExternalTransaction{hash: hash}) do
    hash
    |> to_string()
    |> QRCode.to_png()
    |> Base.encode64()
  ...
  def to_address_hash(%ExternalTransaction{to_address_hash: nil, created_contract_address_hash: address_hash}),
    do: address_hash

  def to_address_hash(%ExternalTransaction{to_address_hash: address_hash}), do: address_hash

  def transaction_display_type(%ExternalTransaction{} = transaction) do
    gettext("External Transaction")
  end

  def type_suffix(%ExternalTransaction{} = transaction) do
    "transaction" # change this to change the color
  end
  ...
  * `:include_label` - Boolean. Defaults to true. Flag for displaying unit with value.
  ...
  @spec decoded_revert_reason(ExternalTransaction.t() | nil) :: binary() | nil
