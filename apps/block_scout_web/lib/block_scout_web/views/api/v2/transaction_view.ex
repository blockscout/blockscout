  ...
  def prepare_log_mapping(mapping) do
    Enum.map(mapping, fn {name, type, indexed?, value} ->
      %{
        ...
        "type" => type,
        "indexed" => indexed?,
        "value" => ABIEncodedValueView.value_json(type, value)
      }
    end)
  end
  ...
  case @chain_type do
    :polygon_edge ->
      ...
      defp chain_type_fields(result, transaction, single_transaction?, conn, _watchlist_names) do
        if single_transaction? do
          ...
          BlockScoutWeb.API.V2.PolygonEdgeView.extend_transaction_json_response(
            result,
            transaction.hash,
            conn
          )
    ...
    :polygon_zkevm ->
      ...
      defp chain_type_fields(result, transaction, single_transaction?, _conn, _watchlist_names) do
        if single_transaction? do
          # credo:disable-for-next-line Credo.Check.Design.AliasUsage
          BlockScoutWeb.API.V2.PolygonZkevmView.extend_transaction_json_response(
            result,
            transaction
          )
        else
