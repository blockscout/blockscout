defmodule BlockScoutWeb.SmartContractView do
  ...
  def values_only(value, type, components) when is_list(value) do
    is_too_long = length(value) > Enum.at(Tuple.to_list(Application.get_env(:block_scout_web, :max_size_to_show_array_as_is)), 0)
    cond do
      String.starts_with?(type, "tuple") ->
        tuple_types =
          type
  ...
  def values_only(value, type, _components) when is_tuple(value) do
    ...
    wrap_output(values, tuple_size(value) > Enum.at(Tuple.to_list(Application.get_env(:block_scout_web, :max_size_to_show_array_as_is)), 0))
  end
