defmodule BlockScoutWeb.Chain.GasUsageHistoryChartController do
  ...
  defp extract_history(db_results) do
    Enum.map(db_results, fn row ->
      %{date: row.date, gas_used: row.gas_used}
    end)
  end

  def encode_transaction_history_data(transaction_history_data) do
    transaction_history_data
    |> Jason.encode()
    |> case do
      {:ok, data} -> data
