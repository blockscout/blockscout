...
defmodule BlockScoutWeb.Chain do
  ...
  @spec next_page_params_for_state_changes(list(), map(), PagingOptions.t()) :: {list(), map() | nil}
  def next_page_params_for_state_changes(list_plus_one, params, %PagingOptions{page_size: page_size}) do
    actual_page_size = page_size - 1
    {items, next_page} = Enum.split(list_plus_one, actual_page_size)
  ...
  """
  @spec paging_options(any) :: [{:paging_options, Explorer.PagingOptions.t()}, ...]
  def paging_options(params) do
    params
    |> do_paging_options()
  ...
  defp do_paging_options(%{"block_number" => block_number_string, "index" => index_string})
  ...
  defp do_paging_options(%{block_number: block_number, index: index}) do
    [paging_options: %{@default_paging_options | key: {block_number, index}}]
  end

  defp do_paging_options(%{"block_number" => block_number_string}) when is_binary(block_number_string) do
  ...
  defp do_paging_options(%{block_number: block_number}) do
    [paging_options: %{@default_paging_options | key: {block_number}}]
  end

  defp do_paging_options(%{"transaction_index" => transaction_index_string, "index" => index_string})
  ...
  defp do_paging_options(%{"transaction_index" => transaction_index, "index" => index})
       when is_integer(transaction_index) and is_integer(index) do
    [paging_options: %{@default_paging_options | key: %{transaction_index: transaction_index, index: index}}]
  end

  defp do_paging_options(%{transaction_index: transaction_index, index: index}) do
    [paging_options: %{@default_paging_options | key: %{transaction_index: transaction_index, index: index}}]
  end

  defp do_paging_options(%{"index" => index_string}) when is_binary(index_string) do
  ...
  defp do_paging_options(%{"index" => index}) when is_integer(index) do
    [paging_options: %{@default_paging_options | key: {index}}]
  end

  defp do_paging_options(%{index: index}) do
    [paging_options: %{@default_paging_options | key: {index}}]
  end

  defp do_paging_options(%{"nonce" => nonce_string}) when is_binary(nonce_string) do
  ...
  defp do_paging_options(%{"nonce" => nonce}) when is_integer(nonce) do
    [paging_options: %{@default_paging_options | key: {nonce}}]
  end

  defp do_paging_options(%{nonce: nonce}) do
    [paging_options: %{@default_paging_options | key: {nonce}}]
  end

  defp do_paging_options(%{"number" => number_string}) when is_binary(number_string) do
  ...
  defp do_paging_options(%{"number" => number}) when is_integer(number) do
    [paging_options: %{@default_paging_options | key: {number}}]
  end

  defp do_paging_options(%{number: number}) do
    [paging_options: %{@default_paging_options | key: {number}}]
  end

  defp do_paging_options(%{"inserted_at" => inserted_at_string, "hash" => hash_string})
  ...
  defp do_paging_options(%{inserted_at: inserted_at, hash: hash_string}) when is_binary(hash_string) do
    case string_to_full_hash(hash_string) do
      {:ok, hash} ->
        [paging_options: %{@default_paging_options | key: {inserted_at, hash}, is_pending_transaction: true}]
  ...
  defp do_paging_options(%{"token_name" => name, "token_type" => type, "token_inserted_at" => inserted_at}),
    do: [paging_options: %{@default_paging_options | key: {name, type, inserted_at}}]

  defp do_paging_options(%{token_name: name, token_type: type, token_inserted_at: inserted_at}),
    do: [paging_options: %{@default_paging_options | key: {name, type, inserted_at}}]

  defp do_paging_options(%{"value" => value, "address_hash" => address_hash}) do
    [paging_options: %{@default_paging_options | key: {value, address_hash}}]
  end

  defp do_paging_options(%{value: "", address_hash: address_hash}) do
    [paging_options: %{@default_paging_options | key: {nil, address_hash}}]
  end

  defp do_paging_options(%{value: "null", address_hash: address_hash}) do
    [paging_options: %{@default_paging_options | key: {nil, address_hash}}]
  end
  ...
  defp do_paging_options(%{"fiat_value" => fiat_value_string, "value" => value_string, "id" => id_string})
  ...
  defp do_paging_options(%{fiat_value: fiat_value_string, value: value_string, id: id})
  ...
  defp do_paging_options(%{"value" => value_string, "id" => id_string})
  ...
  defp do_paging_options(%{value: value, id: id}) do
    [paging_options: %{@default_paging_options | key: {nil, value, id}}]
  end

  defp do_paging_options(%{"state_changes_count" => state_changes_count_string})
  ...
  defp do_paging_options(%{state_changes_count: state_changes_count}) when is_integer(state_changes_count) do
    [paging_options: %{@default_paging_options | key: {state_changes_count}}]
  end

  defp do_paging_options(%{"l1_block_number" => block_number, "transaction_hash" => transaction_hash}) do
  ...
  defp do_paging_options(%{"id" => id_string}) when is_binary(id_string) do
  ...
  defp do_paging_options(%{"id" => id}) when is_integer(id) do
    [paging_options: %{@default_paging_options | key: {id}}]
  end
  ...
  defp do_paging_options(%{id: id}) do
    [paging_options: %{@default_paging_options | key: {id}}]
  end
  ...
  defp do_paging_options(%{timestamp: timestamp, init_transaction_hash: init_transaction_hash}) do
  ...
  defp do_paging_options(%{token_contract_address_hash: token_contract_address_hash, token_type: token_type}) do
  ...
  defp do_paging_options(_params), do: [paging_options: @default_paging_options]
  ...
  @doc """
    Splits a list fetched with n+1 strategy by the actual page size from paging_options,
    and generates next_page_params if there are more items.

    Returns `{items, next_page_params | nil}`.

    ## Options
      * `:paging_function` - custom function to extract paging params from the last item.
        Defaults to `&paging_params/1`.
  """
  @spec paginate_list(list(), map(), PagingOptions.t(), keyword()) :: {list(), map() | nil}
  def paginate_list(list_plus_one, params, paging_options, opts \\ [])

  def paginate_list([], _params, _paging_options, _opts), do: {[], nil}

  def paginate_list(list_plus_one, params, %PagingOptions{page_size: page_size}, opts) do
    actual_page_size = page_size - 1
    {items, next_page} = Enum.split(list_plus_one, actual_page_size)
    ...
    next_page_params =
      case next_page do
        ...
        _ ->
          paging_function = Keyword.get(opts, :paging_function, &paging_params/1)
          paging_params = paging_function.(List.last(items))
  ...
  def paging_params({%Token{} = token, _}) do
    paging_params(token)
  end

  def paging_params(%TagAddress{id: id}) do
    %{"id" => id}
  end

  def paging_params(%TagTransaction{id: id}) do
    %{"id" => id}
  end

  def paging_params(%WatchlistAddress{id: id}) do
    %{"id" => id}
  end
  ...
  def paging_params({%Reward{block: %{number: number}}, _}) do
    %{"block_number" => number, "index" => 0}
  end

  def paging_params(%Block{number: number}) do
    %{block_number: number}
  end

  def paging_params(%InternalTransaction{
        index: index,
        transaction_index: transaction_index,
        block_number: block_number
      }) do
  ...
  def paging_params(%Log{index: index, block_number: block_number}) do
    %{block_number: block_number, index: index}
  end

  def paging_params(%Transaction{block_number: nil, inserted_at: inserted_at, hash: hash}) do
    %{inserted_at: DateTime.to_iso8601(inserted_at), hash: hash}
  end

  def paging_params(%Transaction{block_number: block_number, index: index}) do
    %{block_number: block_number, index: index}
  end

  def paging_params(%TokenTransfer{block_number: block_number, log_index: index}) do
    %{block_number: block_number, index: index}
  end

  def paging_params(%Address.Token{name: name, type: type, inserted_at: inserted_at}) do
  ...
  def paging_params(%CurrentTokenBalance{address_hash: address_hash, value: value}) when is_nil(value) do
    %{address_hash: to_string(address_hash), value: nil}
  end

  def paging_params(%CurrentTokenBalance{address_hash: address_hash, value: value}) do
    %{address_hash: to_string(address_hash), value: to_string(Decimal.to_integer(value))}
  end

  def paging_params(%CoinBalance{block_number: block_number}) do
    %{block_number: block_number}
  end

  def paging_params(%SmartContract{address: %NotLoaded{}} = smart_contract) do
    %{smart_contract_id: smart_contract.id}
  end

  def paging_params(%OptimismFrameSequence{id: id}) do
    %{id: id}
  end

  def paging_params(%OptimismOutputRoot{l2_output_index: index}) do
    %{index: index}
  end

  def paging_params(%OptimismInteropMessage{timestamp: timestamp, init_transaction_hash: init_transaction_hash}) do
    %{timestamp: DateTime.to_unix(timestamp), init_transaction_hash: init_transaction_hash}
  end

  def paging_params(%SmartContract{} = smart_contract) do
    %{
      smart_contract_id: smart_contract.id,
  ...
  def paging_params(%ScrollBatch{number: number}) do
    %{number: number}
  end

  def paging_params(%ScrollBridge{index: id}) do
    %{id: id}
  end

  def paging_params(%Instance{token_id: token_id}) do
    %{"unique_token" => Decimal.to_integer(token_id)}
  end

  def paging_params(%{index: index}) do
    %{index: index}
  end

  # clause for zkEVM batches pagination
  def paging_params(%{number: number}) do
    %{"number" => number}
  end
  ...
  def paging_params(%{l1_block_number: block_number, user: _}) do
    %{block_number: block_number}
  end

  # Shibarium Withdrawals — kept next to deposits for symmetry; :user is the discriminator.
  def paging_params(%{l2_block_number: block_number, user: _}) do
    %{block_number: block_number}
  end

  # clause for Optimism Deposits
  def paging_params(%{l1_block_number: l1_block_number, l2_transaction_hash: l2_transaction_hash}) do
    %{l1_block_number: l1_block_number, transaction_hash: l2_transaction_hash}
  end

  # clause for Optimism Withdrawals
  def paging_params(%{msg_nonce: nonce}) do
    %{nonce: nonce}
  end
  ...
  @spec token_transfers_next_page_params(list(), map(), PagingOptions.t()) :: {list(), map() | nil}
  def token_transfers_next_page_params(list_plus_one, params, %PagingOptions{page_size: page_size}) do
    actual_page_size = page_size - 1
    {items, next_page} = Enum.split(list_plus_one, actual_page_size)
    ...
    next_page_params =
      case next_page do
        ...
        _ ->
          ...
          current_token_transfer = List.last(items)
          ...
          if next_token_transfer.log_index == current_token_transfer.log_index and
            ...
            new_params = paging_params(List.last(items))
            string_keys = map_to_string_keys(new_params)
